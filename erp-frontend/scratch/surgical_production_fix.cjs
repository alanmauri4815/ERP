...
// 1. Precise DOM Fix (Lines 503 to 567 approximately)
const startTag = `<div style="background: rgba(var(--primary-rgb), 0.05); padding: 1rem; border-radius: 0.5rem; margin-bottom: 1.5rem; border: 1px dashed var(--primary)">`;
const endTag = `<table class="item-table">`;
...
`;
    content = content.substring(0, startIndex) + newFinancialSection + content.substring(endIndex);
}
