        ...
        if (purchase.items && purchase.items.length > 0) {
            purchase.items.forEach(item => {
                formatted.push({
                    ...
                    'Glosa': purchase.description || '-',
                    'Articulo': item.product_code || item.mp_code || '-',
                    'Nombre Articulo': item.product_name || item.mp_name || item.custom_name || '-',
                    'Cantidad': item.quantity,
                    'Precio': item.unit_price || item.price,
                    'Subtotal': item.subtotal || (item.quantity * item.price),
...
export function formatLibroComprasForExport(purchases) {
    return purchases.map(p => {
        ...
        return {
            ...
            'Cotizacion': p.quotation_name || p.project_name || '-',
            'Neto': neto,
            'IVA': iva,
            'Total': total,
            'Pagado': pagado,
