...
async function inspectCxp(empresaId = 1) {
    console.log(`--- Inspección de Cuentas por Pagar (2.1.01) para empresa ${empresaId} ---`);
    
    // 1. Obtener todos los movimientos de la cuenta 2.1.01
    ...
    for (const [ref, b] of Object.entries(balances)) {
        ...
        if (Math.abs(saldo) > 0) {
            console.log(`Ref: ${ref} | Haber: ${b.haber} | Debe: ${b.debe} | SALDO: ${saldo}`);
